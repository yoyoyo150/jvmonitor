...
母父: {motherFather}";
        ...
        // DBからこの馬の全レースの馬印データを読み込む
        // DBからこの馬の全レースの馬印データを読み込む
        private void LoadAllMarksFromDatabase()
        {
            var normalizedHorseNameForDebug = NormalizeHorseName(_horseName);
            System.Windows.Forms.MessageBox.Show($"デバッグ情報:\n\nこれからこの馬のデータを検索します。\n\n・元の馬名: '{_horseName}'\n・検索用の馬名: '{normalizedHorseNameForDebug}'", "デバッグ: 検索する馬名");
            ...
            try
            {
                ...
                conn.Open();
                System.Diagnostics.Debug.WriteLine("excel_data.db接続成功");

                var normalizedHorseName = NormalizeHorseName(_horseName);
                if (string.IsNullOrEmpty(normalizedHorseName)) return;

                var tableQuery = "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'EXCEL_DATA_%' ORDER BY name DESC";
                using var tableCmd = new SQLiteCommand(tableQuery, conn);
                using var tableReader = tableCmd.ExecuteReader();
                
                var tableNames = new List<string>();
                while (tableReader.Read()) tableNames.Add(tableReader["name"].ToString()!);

                var columnsToLoad = new List<string> {
                    "R印1", "R印2", "R印3",
                    "馬印1", "馬印2", "馬印3", "馬印4", "馬印5", "馬印6", "馬印7", "馬印8",
                    "前脚質", "前馬印1", "前馬印2", "前馬印3", "前馬印4", "前馬印5", "前馬印6", "前馬印7", "前馬印8",
                    "前人気", "ZI指数", "ZM"
                };
                ...
                foreach (var tableName in tableNames)
                {
                    try
                    {
                        ...
                        using (var schemaCmd = new SQLiteCommand($"PRAGMA table_info({tableName})", conn))
                        using (var schemaReader = schemaCmd.ExecuteReader())
                        {
                            while (schemaReader.Read()) availableColumns.Add(schemaReader["name"].ToString());
                        }

                        var selectColumns = columnsToLoad.Where(c => availableColumns.Contains(c)).ToList();
                        if (!selectColumns.Any()) continue;

                        var sql = $"SELECT 馬名S, {string.Join(", ", selectColumns)} FROM {tableName} WHERE 馬名S LIKE @horseName";
                        
                        using var cmd = new SQLiteCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@horseName", $"%{normalizedHorseName}%");
                        
                        using var reader = cmd.ExecuteReader();
                        if (reader.Read())
                        {
                            var dateStr = tableName.Replace("EXCEL_DATA_", "");
                            var raceKey = $"{dateStr}_{normalizedHorseName}"; 

                            var marksResult = new HorseMarksResult();
                            
                            string GetValue(string col) => selectColumns.Contains(col) ? reader[col]?.ToString() : null;

                            marksResult.R_MARK1 = GetValue("R印1");
                            marksResult.R_MARK2 = GetValue("R印2");
                            marksResult.R_MARK3 = GetValue("R印3");
                            marksResult.Mark1 = GetValue("馬印1");
                            marksResult.Mark2 = GetValue("馬印2");
                            marksResult.Mark3 = GetValue("馬印3");
                            marksResult.Mark4 = GetValue("馬印4");
                            marksResult.Mark5 = GetValue("馬印5");
                            marksResult.Mark6 = GetValue("馬印6");
                            marksResult.Mark7 = GetValue("馬印7");
                            marksResult.Mark8 = GetValue("馬印8");
                            marksResult.PREV_KYAKUSHITSU = GetValue("前脚質");
                            marksResult.PREV_MARK1 = GetValue("前馬印1");
                            marksResult.PREV_MARK2 = GetValue("前馬印2");
                            marksResult.PREV_MARK3 = GetValue("前馬印3");
                            marksResult.PREV_MARK4 = GetValue("前馬印4");
                            marksResult.PREV_MARK5 = GetValue("前馬印5");
                            marksResult.PREV_MARK6 = GetValue("前馬印6");
                            marksResult.PREV_MARK7 = GetValue("前馬印7");
                            marksResult.PREV_MARK8 = GetValue("前馬印8");
                            marksResult.PREV_NINKI = GetValue("前人気");
                            marksResult.ZI_INDEX = GetValue("ZI指数");
                            marksResult.ZM_VALUE = GetValue("ZM");
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"テーブル {tableName} の読み込みエラー: {ex.Message}");
                        continue;
                    }
                }
                System.Windows.Forms.MessageBox.Show($"デバッグ情報:\n\n`excel_data.db` から読み込んだ馬のレースデータ件数: {_allHorseMarks.Count} 件", "デバッグ: LoadAllMarksFromDatabase");
            }
            catch (Exception ex)
            {
