...
class FinalStablePatchV4:
    """最終安定パッチ v4 - 無限ループを物理的に止める"""
    ...
    def create_roi_lock_v4(self):
        ...
        try:
            ...
            # ROI対象フラグを追加
            try:
                cursor.execute("ALTER TABLE SE_FE ADD COLUMN roi_lock_v4 INTEGER")
                print("roi_lock_v4列追加")
            except:
                print("roi_lock_v4列は既に存在")
            
            # 全件にROI対象フラグを設定
            cursor.execute("UPDATE SE_FE SET roi_lock_v4 = 1")
            print("ROI対象フラグ設定完了")
            
            # ROI計算用ビューを作成（ZI/ZM完全遮断）
            cursor.execute("""
            DROP VIEW IF EXISTS V_ROI_INPUT_V4
            """)
            
            cursor.execute("""
            CREATE VIEW V_ROI_INPUT_V4 AS
            SELECT
            ...
            print(f"列ロック v4作成エラー: {e}")
            return False
    
    def calculate_roi_fixed_v4(self, start_date="2024-11-02", end_date="2025-09-28"):
        """2) ROI計算 v4（円ベースのみ／ZI・ZM完全遮断）"""
        print("=== ROI計算 v4（固定列のみ） ===")
        ...
        try:
            ...
            print(f"固定列ROI計算エラー v4: {e}")
            return None
    
    def check_plausible_range_v4(self, kpi_results):
        """3) 常識レンジで強制ブレーキ v4"""
        print("=== 常識レンジ監査 v4 ===")
        ...
        try:
            ...
            print(f"常識レンジ監査エラー v4: {e}")
            return False
    
    def show_sample_data_v4(self):
        """4) サンプルデータ表示 v4"""
        print("=== サンプルデータ表示 v4 ===")
        
        try:
            # V_ROI_INPUT_V4の小さなサンプル（10行）
            query_sample = """
            SELECT 
              race_id,
              horse_id,
            ...
            print("V_ROI_INPUT_V4サンプル（10行）:")
            print(result_sample.to_string(index=False))
            
            return result_sample
            
        except Exception as e:
            print(f"サンプルデータ表示エラー v4: {e}")
            return None
    
    def run_final_stable_patch_v4(self, start_date="2024-11-02", end_date="2025-09-28"):
        """最終安定パッチ v4実行"""
        print("=== 最終安定パッチ v4実行 ===")
        
        try:
            # 1) 列ロック v4作成
            if not self.create_roi_lock_v4():
                return False
            
            # 2) ROI計算 v4（固定列のみ）
            kpi_results = self.calculate_roi_fixed_v4(start_date, end_date)
            if kpi_results is None:
                return False
            
            # 3) 常識レンジ監査 v4
            if not self.check_plausible_range_v4(kpi_results):
                print("❌ 常識レンジ外のため停止")
                return False
            
            # 4) サンプルデータ表示 v4
            self.show_sample_data_v4()
            
            print("✅ 最終安定パッチ v4完了")
            return True
            
        except Exception as e:
            print(f"最終安定パッチ v4エラー: {e}")
            return False

def main():
    patch = FinalStablePatchV4()
    success = patch.run_final_stable_patch_v4()
