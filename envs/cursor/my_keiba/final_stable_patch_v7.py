...
class FinalStablePatchV7:
    """最終安定パッチ v7 - 無限ループを物理的に止める"""
    ...
    def create_roi_lock_v7(self):
        ...
        try:
            ...
            # ROI対象フラグを追加
            try:
                cursor.execute("ALTER TABLE SE_FE ADD COLUMN roi_lock_v7 INTEGER")
                print("roi_lock_v7列追加")
            except:
                print("roi_lock_v7列は既に存在")
            
            # 全件にROI対象フラグを設定
            cursor.execute("UPDATE SE_FE SET roi_lock_v7 = 1")
            print("ROI対象フラグ設定完了")
            
            # ROI計算用ビューを作成（ZI/ZM完全遮断）
            cursor.execute("""
            DROP VIEW IF EXISTS V_ROI_INPUT_V7
            """)
            
            cursor.execute("""
            CREATE VIEW V_ROI_INPUT_V7 AS
            SELECT
            ...
            print(f"列ロック v7作成エラー: {e}")
            return False
    
    def calculate_roi_fixed_v7(self, start_date="2024-11-02", end_date="2025-09-28"):
        """2) ROI計算 v7（円ベースのみ／ZI・ZM完全遮断）"""
        print("=== ROI計算 v7（固定列のみ） ===")
        ...
        try:
            ...
            print(f"固定列ROI計算エラー v7: {e}")
            return None
    
    def check_plausible_range_v7(self, kpi_results):
        """3) 常識レンジで強制ブレーキ v7"""
        print("=== 常識レンジ監査 v7 ===")
        ...
        try:
            ...
            print(f"常識レンジ監査エラー v7: {e}")
            return False
    
    def show_sample_data_v7(self):
        """4) サンプルデータ表示 v7"""
        print("=== サンプルデータ表示 v7 ===")
        
        try:
            # V_ROI_INPUT_V7の小さなサンプル（10行）
            query_sample = """
            SELECT 
              race_id,
              horse_id,
            ...
            print("V_ROI_INPUT_V7サンプル（10行）:")
            print(result_sample.to_string(index=False))
            
            return result_sample
            
        except Exception as e:
            print(f"サンプルデータ表示エラー v7: {e}")
            return None
    
    def run_final_stable_patch_v7(self, start_date="2024-11-02", end_date="2025-09-28"):
        """最終安定パッチ v7実行"""
        print("=== 最終安定パッチ v7実行 ===")
        
        try:
            # 1) 列ロック v7作成
            if not self.create_roi_lock_v7():
                return False
            
            # 2) ROI計算 v7（固定列のみ）
            kpi_results = self.calculate_roi_fixed_v7(start_date, end_date)
            if kpi_results is None:
                return False
            
            # 3) 常識レンジ監査 v7
            if not self.check_plausible_range_v7(kpi_results):
                print("❌ 常識レンジ外のため停止")
                return False
            
            # 4) サンプルデータ表示 v7
            self.show_sample_data_v7()
            
            print("✅ 最終安定パッチ v7完了")
            return True
            
        except Exception as e:
            print(f"最終安定パッチ v7エラー: {e}")
            return False

def main():
    patch = FinalStablePatchV7()
    success = patch.run_final_stable_patch_v7()
