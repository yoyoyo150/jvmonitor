...
class FixWinPayYenUltimate:
    ...
    def fix_win_pay_yen_ultimate(self):
        """win_pay_yen究極修正（倍率×100円）"""
        ...
        try:
            ...
            self.conn.commit()
            print("win_pay_yen究極修正完了（倍率×100円）")
    ...
    def test_roi_calculation_ultimate(self, test_date="2024-11-02"):
        """ROI計算テスト（究極版）"""
        print(f"=== ROI計算テスト（究極版）（{test_date}） ===")
        ...
        try:
            ...
            print(f"ROI計算テスト（究極版）エラー: {e}")
            return False
    
    def run_ultimate_fix(self):
        """究極修正実行"""
        print("=== win_pay_yen究極修正実行 ===")
        
        try:
            # 1) 究極修正実行
            if not self.fix_win_pay_yen_ultimate():
                return False
            ...
            # 3) ROI計算テスト（究極版）
            if not self.test_roi_calculation_ultimate():
                return False
            
            print("✅ win_pay_yen究極修正完了")
            return True
            
        except Exception as e:
            print(f"win_pay_yen究極修正実行エラー: {e}")
            return False

def main():
    fixer = FixWinPayYenUltimate()
