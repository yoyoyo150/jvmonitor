...
class CorrectTrainerPredictionComparison:
    """正しい調教師予想手法の比較クラス"""
    ...
    def calculate_september_performance(self, trainer_list, top_n=20):
        """9月の成績計算"""
        try:
            print(f"\n=== 9月の成績計算（上位{top_n}名） ===")
    ...
    def compare_methods(self):
        ...
        try:
            ...
            if mark_trainers is not None:
                mark_results = self.calculate_september_performance(mark_trainers, top_n=20)
            else:
                mark_results = None
            ...
            if nine_month_trainers is not None:
                nine_month_results = self.calculate_september_performance(nine_month_trainers, top_n=20)
            else:
...
def main():
    """メイン実行"""
    comparison = CorrectTrainerPredictionComparison()
