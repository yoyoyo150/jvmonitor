...
class CorrectUnitAnalyzer:
    ...
    def analyze_non_zero_data(self):
        """非0データの詳細分析"""
        print("=== 非0データの詳細分析 ===")
    ...
    def determine_correct_units(self, result_zi, result_zm):
        ...
        try:
            ...
            # ZI_Indexは「円」の可能性が高い（平均値が100前後、円らしい割合が高い）
            if zi_avg < 200 and zi_yen_ratio > 0.5:
                zi_is_yen = True
                print("  → 円として判定（平均値が100前後、円らしい割合が高い）")
            else:
                zi_is_yen = False
                print("  → 倍率として判定")
            ...
            # ZM_Valueは「円」の可能性が高い（平均値が700前後、円らしい割合が高い）
            if zm_avg > 500 and zm_yen_ratio > 0.5:
                zm_is_yen = True
                print("  → 円として判定（平均値が700前後、円らしい割合が高い）")
            else:
                zm_is_yen = False
                print("  → 倍率として判定")
    ...
    def calculate_correct_kpi(self, start_date="2024-11-02", end_date="2025-09-28"):
        """正しいKPI計算"""
        print("\n=== 正しいKPI計算 ===")
    ...
    def run_correct_analysis(self):
        ...
        # 1) 非0データの詳細分析
        result_zi, result_zm = self.analyze_non_zero_data()
        ...
        # 4) 正しいKPI計算
        if not self.calculate_correct_kpi():
            return False
...
def main():
    analyzer = CorrectUnitAnalyzer()
    success = analyzer.run_correct_analysis()
