...
# -*- coding: utf-8 -*-
"""
Excelファイル内のMark5/Mark6データ確認スクリプト
"""
import pandas as pd
import sys
...
def check_excel_marks_data(file_path: Path, sheet_name=0):
    """Excelファイル内のMark5/Mark6データをチェック"""
    ...
    try:
        ...
        # トウシンマカオのデータ（もしあれば）と、Mark5/Mark6のサンプルを表示
        print("\n--- Mark5/Mark6データのサンプル --- ")
        ...
        # トウシンマカオのデータを確認
        if "馬名" in df.columns:
            toshin_macao_df = df[df["馬名"] == 'トウシンマカオ']
            if not toshin_macao_df.empty:
                print("\n--- トウシンマカオのMark5/Mark6データ --- ")
                print(toshin_macao_df[mark_columns].to_string())
            else:
                print("\nトウシンマカオのデータはファイルに見つかりませんでした。")
        else:
            print("Excelファイルに '馬名' カラムが見つかりません。")
...
if __name__ == "__main__":
    ...
    check_excel_marks_data(target_file)
