...
class UltimateFinalFix:
    ...
    def fix_win_pay_yen_correctly(self):
        ...
        try:
            ...
            self.conn.commit()
            print("win_pay_yen正しく修正完了（倍率×100円）")
    ...
    def verify_correct_fix(self):
        ...
        try:
            ...
            print(f"正しい修正検証エラー: {e}")
            return None, None
    
    def calculate_correct_roi(self, start_date="2024-11-02", end_date="2025-09-28"):
        """正しいROI計算"""
        print("=== 正しいROI計算 ===")
    ...
    def run_ultimate_fix(self):
        ...
        # 2) 正しい修正の検証
        stats, sample = self.verify_correct_fix()
        if stats is None:
            return False
        
        # 3) 正しいROI計算
        if not self.calculate_correct_roi():
            return False
