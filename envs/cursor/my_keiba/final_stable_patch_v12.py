...
class FinalStablePatchV12:
    """最終安定パッチ v12 - 無限ループを物理的に止める"""
    ...
    def create_roi_lock_v12(self):
        ...
        try:
            ...
            # ROI対象フラグを追加
            try:
                cursor.execute("ALTER TABLE SE_FE ADD COLUMN roi_lock_v12 INTEGER")
                print("roi_lock_v12列追加")
            except:
                print("roi_lock_v12列は既に存在")
            
            # 全件にROI対象フラグを設定
            cursor.execute("UPDATE SE_FE SET roi_lock_v12 = 1")
            print("ROI対象フラグ設定完了")
            
            # ROI計算用ビューを作成（ZI/ZM完全遮断）
            cursor.execute("""
            DROP VIEW IF EXISTS V_ROI_INPUT_V12
            """)
            
            cursor.execute("""
            CREATE VIEW V_ROI_INPUT_V12 AS
            SELECT
              ...
              roi_lock_v12
            FROM SE_FE
            WHERE roi_lock_v12 = 1
            """)
            print("V_ROI_INPUT_V12ビュー作成完了")
            ...
            print(f"列ロック v12作成エラー: {e}")
            return False
    
    def calculate_roi_fixed_v12(self, start_date="2024-11-02", end_date="2025-09-28"):
        """2) ROI計算 v12（円ベースのみ／ZI・ZM完全遮断）"""
        print("=== ROI計算 v12（固定列のみ） ===")
        ...
        try:
            ...
            print("固定列ROI計算結果 v12:")
            print(kpi_results.to_string(index=False))
            
            return kpi_results
            
        except Exception as e:
            print(f"固定列ROI計算エラー v12: {e}")
            return None
    
    def check_plausible_range_v12(self, kpi_results):
        """3) 常識レンジで強制ブレーキ v12"""
        print("=== 常識レンジ監査 v12 ===")
        ...
        try:
            ...
            print(f"常識レンジ監査エラー v12: {e}")
            return False
    
    def show_sample_data_v12(self):
        """4) サンプルデータ表示 v12"""
        print("=== サンプルデータ表示 v12 ===")
        
        try:
            # V_ROI_INPUT_V12の小さなサンプル（10行）
            query_sample = """
            SELECT 
              race_id,
              horse_id,
            ...
            print("V_ROI_INPUT_V12サンプル（10行）:")
            print(result_sample.to_string(index=False))
            
            return result_sample
            
        except Exception as e:
            print(f"サンプルデータ表示エラー v12: {e}")
            return None
    
    def run_final_stable_patch_v12(self, start_date="2024-11-02", end_date="2025-09-28"):
        """最終安定パッチ v12実行"""
        print("=== 最終安定パッチ v12実行 ===")
        
        try:
            # 1) 列ロック v12作成
            if not self.create_roi_lock_v12():
                return False
            
            # 2) ROI計算 v12（固定列のみ）
            kpi_results = self.calculate_roi_fixed_v12(start_date, end_date)
            if kpi_results is None:
                return False
            
            # 3) 常識レンジ監査 v12
            if not self.check_plausible_range_v12(kpi_results):
                print("❌ 常識レンジ外のため停止")
                return False
            
            # 4) サンプルデータ表示 v12
            self.show_sample_data_v12()
            
            print("✅ 最終安定パッチ v12完了")
            return True
            
        except Exception as e:
            print(f"最終安定パッチ v12エラー: {e}")
            return False

def main():
    patch = FinalStablePatchV12()
    success = patch.run_final_stable_patch_v12()
