...
def fix_mark1_import():
    ...
    print(f"1. {latest_file} の馬印1データを確認")
    try:
    ...
    print(f"馬印1が空でないレコード: {len(non_empty_mark1)} 件")
    
    if len(non_empty_mark1) > 0:
        print("馬印1が空でないレコードの詳細:")
        for i, row in non_empty_mark1.head(5).iterrows():
            print(f"  {row['馬名S']}: 馬印1={row['馬印1']}, 馬印2={row['馬印2']}, 馬印3={row['馬印3']}")
