...
class UltimateCorrectFix:
    ...
    def analyze_correct_units(self):
        """正しい単位の分析"""
        print("=== 正しい単位の分析 ===")
    ...
    def determine_ultimate_units(self, result_zi, result_zm):
        ...
        try:
            ...
            # ZI_Indexは「倍率」の可能性が高い（平均値が70前後、範囲が60-80）
            if zi_avg < 100 and zi_max < 200:
                zi_is_yen = False
                print("  → 倍率として判定（平均値が70前後、範囲が60-80）")
            else:
                zi_is_yen = True
                print("  → 円として判定")
            ...
            # ZM_Valueは「倍率」の可能性が高い（平均値が20前後、範囲が10-30）
            if zm_avg < 100 and zm_max < 200:
                zm_is_yen = False
                print("  → 倍率として判定（平均値が20前後、範囲が10-30）")
            else:
                zm_is_yen = True
                print("  → 円として判定")
    ...
    def run_ultimate_fix(self):
        ...
        # 1) 正しい単位の分析
        result_zi, result_zm = self.analyze_correct_units()
